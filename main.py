...
class GitHubChatGPTPullRequestReviewer:
    ...
    def _config_gh(self) -> None:
        ...
        self.gh_pr_url = f"{gh_api_url}/repos/{self.gh_repo_name}/pulls/{self.gh_pr_id}"
        self.gh_headers = {
    ...
    def _config_openai(self) -> None:
        ...
        self.openai_temperature = float(os.environ.get("OPENAI_TEMPERATURE", "0.5"))
        self.openai_max_tokens = int(os.environ.get("OPENAI_MAX_TOKENS", "2048"))
        self.openai_extra_criteria = os.environ.get("OPENAI_EXTRA_CRITERIA", "").strip()

        # SDK v1 client (uses OPENAI_API_KEY from env)
        self._openai = OpenAI()

        self.chatgpt_initial_instruction = (
            "You are a GitHub PR reviewer bot. You will receive a text containing the "
            "diff from a PR with all proposed changes. Analyze it and return suggestions "
            ...
            "- prefer explicit over implicit (e.g., in Python, avoid `from x import *`)\n"
            f"{self._prepare_extra_criteria(self.openai_extra_criteria)}\n"
            "Return your response in Markdown. If everything looks good, just say: \"LGTM!\""
        )
    ...
    def get_pr_content(self) -> str:
        resp = requests.get(self.gh_pr_url, headers=self.gh_headers, timeout=60)
    ...
    def get_diff(self) -> Dict[str, str]:
        """
        Returns a mapping {filename: diff_text_for_that_file}.
        """
        # Touch PR to ensure it exists (also used later to comment)
        _ = self.gh_api.get_repo(self.gh_repo_name).get_pull(int(self.gh_pr_id))
    ...
    def pr_review(self, pr_diff: Dict[str, str]) -> List[str]:
        ...
        system_message = {"role": "system", "content": self.chatgpt_initial_instruction}
        results: List[str] = []

        for filename, diff in pr_diff.items():
            message_diff = f"file:\n```{filename}```\ndiff:\n```{diff}```"
            messages = [system_message, {"role": "user", "content": message_diff}]

            # crude token estimate for logging
            print("Estimated tokens:", int(len(self.chatgpt_initial_instruction + message_diff) / 4))
